...
```
直接通过上面的例子可以看出，**有类型值为空的接口不为空**，即`a == nil`为false，因为它已经有类型string了，只是值为零。
interface只有当类型和值都为零的时候，才为nil。

#### gorm报错reflect: reflect.Value.SetInt using unaddressable value 
报unaddressable value，主要原因是在使用reflect.Value反射时，用到了不能地址化的值。
go语言圣经里面，讲了如何使用[reflect.Value修改值](https://books.studygolang.com/gopl-zh/ch12/ch12-05.html)。
根据下面的例子，可以看到常量或者拷贝值，都是unaddressable value.
```
...
#### 什么是go.sum
相比于go.mod，关于go.sum的内容明显少得多。自然，go.mod 的重要性不言而喻，这个文件几乎提供了依赖版本的全部信息。而 go.sum 则是记录了所有依赖的 module 的校验信息，以防下载的依赖被恶意篡改，主要用于安全校验。
...
#### http 响应压缩
HTTP 响应头 Content-Encoding
Content-Encoding：指示用于压缩响应主体的编码方法。常见的值包括 gzip、deflate、br（Brotli），以及没有压缩的 identity。
当 HTTP 响应头中包含 Content-Encoding: gzip 时，表示响应内容已经被 GZIP 压缩。客户端需要支持并识别该编码，以便正确解压缩和处理响应内容。

在未压缩的情况下，HTTP 响应可能如下：
```
HTTP/1.1 200 OK
...
```
下面是在cache里面，对key进行写操作，其中存在两次对loaded判断：
1. 第一次判断如果key是否存在
2. 第二次判断能保证线程安全，如果在第一次loaded后，有其它线程进行写操作，那么当前线程就会变为读操作，以此保证线程安全。

#### 使用锁时，方法接受者为指针
在 Go 语言中，如果一个结构体包含 `sync.Mutex` 字段，其方法的接收者最好是指针。这主要是因为 `sync.Mutex` 的锁定状态不能被复制，所以使用指针可以确保方法操作的是同一个 `Mutex` 实例，而不是其副本。

**例子**

假设有一个结构体 `Counter` 包含一个 `sync.Mutex` 字段：
...
在这种情况下，使用指针接收者 (`*Counter`) 可以确保 `Increment` 和 `Value` 方法对同一个 `Mutex` 实例进行操作，从而保证正确的并发行为。
...
在这种情况下，每次方法调用都会复制 `Counter` 实例，包括 `sync.Mutex` 字段。这意味着每个方法调用都会操作一个不同的 `Mutex` 副本，导致并发控制失效。

**总结**

为了确保包含 `sync.Mutex` 字段的结构体能够正确地处理并发操作，其方法接收者应为指针类型。这确保了所有方法操作的都是同一个 `Mutex` 实例，而不是其副本，从而保证了并发控制的正确性。

#### 选择传参
golang中，使用`func A(args ...interface{})`表示，args可以传入任意数量的参数。应该注意下面的情况：
```
func a(args ...interface{}){}

// 这里的args是类型为[]interface{}，作为a的第一个参数传入
func A(args ...interface{}){
	return a(args)
}
...
#### 初始化顺序导致的bug
背景：在项目中，有部分var 定义的变量需要读取配置中心的数据，来进行初始化。如下场景会因为config为nil panic。
```
var testA = config.getTestA
```
...
```
结论：导致上面场景报错的原因是，go项目的执行顺序是import -> const -> var -> init() -> main()，即var变量的初始化早于config在main函数中的初始化。![流程图](../static/img/go执行顺序1.png)